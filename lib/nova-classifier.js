...
(function () {
  ...
  // Maximum ingredient count for Rules 3b and 4 — products with ≤ this many
  // ingredients are classified as NOVA 1/2 rather than NOVA 3.
  const MIN_PROCESSED_INGREDIENT_COUNT = 3;
  ...
  // Also covers bare food-type names: 'cheese' (all varieties) and 'cheddar' per NOVA spec which
  // lists cheeses as explicit NOVA 3 examples. cheese\s+cultures? is listed before cheese so the
  // longer alternation matches fully (JS alternation is left-to-right, stops at first match).
  const NOVA3_PROCESSING_RE = /\b(cheese\s+cultures?|rennet|cured|curing|smoked|smoking|fermented|fermentation|brined?|pickled|lacto-fermented|acidity\s+regulators?|stabilisers?|humectants?|cheese|cheddar)\b/i;
  ...
  function classifyByIngredients(ingredients) {
    ...
    if (ingredients.length <= MIN_PROCESSED_INGREDIENT_COUNT && hasAddedCulinaryIngredient(ingredients)) {
      return {
        score: 2,
    ...
    if (ingredients.length <= MIN_PROCESSED_INGREDIENT_COUNT) {
      return {
        score: 1,
