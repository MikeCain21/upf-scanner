 ...
 *     artificial sweeteners) + additions from OFF taxonomy alignment (ADR-001, ADR-012).
 ...
 * See ADR-001 and ADR-012 in docs/DECISIONS.md for full rationale.
 *
 * @version 0.5.0
 */
...
(function () {
  ...
  // plus additions from OFF taxonomy alignment (see ADR-001, ADR-012).
  ...
  const NOVA4_E_NUMBERS = new Set([
    ...
    'E249', 'E250', 'E251', 'E252',
    'E282',  // calcium propionate — very common bread preservative
    ...
    'E420',  // sorbitol — sugar-free confectionery
  ...
  const NOVA4_ADDITIVE_PATTERNS = [
    ...
    // -------------------------------------------------------------------
    // Common-name patterns — UK labels often list additives by name without
    // the E-number. These map to E-numbers we already flag above.
    // -------------------------------------------------------------------
    // Emulsifiers / stabilisers / thickeners (name forms of E4xx)
    { pattern: /\bxanthan\s+gum\b/i,               label: 'xanthan gum (E415)'         },
    { pattern: /\bguar\s+gum\b/i,                   label: 'guar gum (E412)'            },
    { pattern: /\bcarrageenan\b/i,                  label: 'carrageenan (E407)'         },
    { pattern: /\bmono-?\s*and\s+di-?glycerides/i,  label: 'mono- and diglycerides (E471)' },
    { pattern: /\bpectin\b/i,                       label: 'pectin (E440)'              },
    // Preservatives (name forms of E2xx)
    { pattern: /\bsodium\s+benzoate\b/i,            label: 'sodium benzoate (E211)'     },
    { pattern: /\bpotassium\s+sorbate\b/i,          label: 'potassium sorbate (E202)'   },
    { pattern: /\bcalcium\s+propionate\b/i,          label: 'calcium propionate (E282)'  },
    { pattern: /\bsodium\s+nitrite\b/i,             label: 'sodium nitrite (E250)'      },
    { pattern: /\bsodium\s+nitrate\b/i,             label: 'sodium nitrate (E251)'      },
    // Sweeteners (name forms of E9xx)
    { pattern: /\bacesulfame\s*(k|potassium)\b/i,   label: 'acesulfame K (E950)'        },
    { pattern: /\bsucralose\b/i,                    label: 'sucralose (E955)'           },
    { pattern: /\baspartame\b/i,                    label: 'aspartame (E951)'           },
    { pattern: /\bsaccharin\b/i,                    label: 'saccharin (E954)'           },
    { pattern: /\bsorbitol\b/i,                     label: 'sorbitol (E420)'            },
    // Flavour enhancers (name form of E621)
    { pattern: /\bmonosodium\s+glutamate\b/i,       label: 'MSG (E621)'                 },
    // New indicators — not covered by existing E-numbers
    // Fructose: naturally in fruit but as a listed ingredient it's industrially
    // refined crystalline fructose. Word boundary prevents matching inside
    // "high-fructose corn syrup" or "glucose-fructose syrup" — those tokens
    // are already caught by earlier patterns in this list.
    { pattern: /\bfructose\b/i,                     label: 'fructose'                   },
    { pattern: /\bpolydextrose\b/i,                 label: 'polydextrose'               },
  ];
